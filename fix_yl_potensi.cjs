...
const regex = /<tr className="bg-slate-900 text-slate-200 font-bold uppercase text-\[9px\]">\s*<th className="p-2 border-r border-slate-800 sticky left-0 bg-slate-900 z-10 min-w-\[70px\]">Tgl<\/th>[\s\S]*?<\/thead>\s*<tbody className="divide-y divide-slate-100 font-bold text-slate-800 text-\[10px\]">[\s\S]*?totRb \+= rb;\s*totBb \+= bb; totPbPagi \+= pbPagi; totPbSore \+= pbSore; totPlgApk \+= plgApk; totSampahBtl \+= sampahBtl; totPlg \+= plg; totRk \+= rk; totRa \+= ra;[\s\S]*?<\/tr>\s*\)\s*\}\)\}\s*<tr className="bg-slate-900 text-amber-300 font-black text-\[10px\]">[\s\S]*?<\/table>/;
                          ...
                          {daysList.map(d => {
                            ...
                            return (
                              <tr key={d} className="hover:bg-indigo-50/50">
                                <td className="p-2 border-r border-slate-100 font-black sticky left-0 bg-white shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] z-10">{dayStrPadded}</td>
