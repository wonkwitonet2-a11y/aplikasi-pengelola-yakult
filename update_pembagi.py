...
new_logic = """app.get("/api/exportRealisasi", async (req, res) => {
  try {
    ...
    // 2. Sheet "Laporan DP1"
    const aoa: any[][] = [];
    // Hapus default pembagi 24 global, jadikan string kosong
    // atau gunakan db.pembagiTanggal jika ada.
    const globalPembagi = db.pembagiTanggal || "";
    aoa.push(["Pembagi", globalPembagi]);
    aoa.push([]);
    aoa.push([]);
    aoa.push([]);
    ...
    // Get breakdownRealisasi for latest month
    let latestBRMonth = "";
    ...
    const ylData = yls.map(yl => {
      ...
      const pembagiArea = breakdownRealisasi[areaCode]?.pembagiTanggal || globalPembagi || 1; // avoid divide by 0 if possible
